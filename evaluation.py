...
testDir        = './training/148' # For example: '/outputs/148'
participantDir = './outputs/148' # For example: '/output/148'
...
def do():
    ...
    print ("################################\n#  Officer evaluation Results\n################################")
    print(' Metrics   | class1  | class2  | class3  | class4  | class5  | class6  | class7  | class8  |  Avg.   |')
    print(' DSC(max=1)|', end='')
    for key, value in dsc.items():
        print (' %2.4f  |'% value, end='')
    mean_dsc_value = float(sum(dsc.values())) / len(dsc)
    print (' %2.4f  |' % mean_dsc_value)

    print(' H95(min=0)|', end='')
    for key, value in h95.items():
        print (' %2.4f  |'% value, end='')
    mean_h95_value = float(sum(h95.values())) / len(h95)
    print (' %2.4f  |' % mean_h95_value)

    print(' VS(max=1) |', end='')
    for key, value in vs.items():
        print (' %2.4f  |'% value, end='')
    mean_vs_value = float(sum(vs.values())) / len(vs)
    print (' %2.4f  |' % mean_vs_value)
