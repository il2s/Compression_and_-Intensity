...
data_dim = 3
ignore_label = 9
num_classes= 9 #from 0 to 8
# crop_size = (48, 64, 64) # 3D
crop_size = (240,240) # 2D
...
def make_one_hot(labels, num_classes):
    ...
    one_hot = torch.cuda.FloatTensor(labels_extend.size(0), num_classes+1, labels_extend.size(2), labels_extend.size(3), labels_extend.size(4)).zero_()
...
def convert2fourclass(label):
    label_processed=np.zeros(label.shape[0:]).astype(np.uint8)
